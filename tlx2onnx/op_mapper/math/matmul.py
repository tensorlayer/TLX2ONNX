...
class MatMul():
    ...
    def version_1(cls, node, **kwargs):
        ...
        if hasattr(node['node'], 'transpose_X'):
        ...
        if hasattr(node['node'], 'transpose_Y'):
        ...
        if node['dtype'] == 'float64':
            m_out = helper.make_tensor_value_info(node['out_nodes_name'][0] + 'm', TensorProto.FLOAT, shape=node['out_tensors'][0])
