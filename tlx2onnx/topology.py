...
def memory_node_act_info():
    # Add processing for the activation function.
    pass
...
def construct_topology(model, inputs):
    ...
    for depth, nodes in enumerate(node_by_depth):
        if depth == 0:
            ...
            for node in nodes:
                if node.layer.__class__.__name__ in tlx.nn.convolution.__all__ + tlx.nn.linear.__all__:
                    if node.layer.act is not None:
                        # Processing for a specific neural network layer.
